  ...
  <div class="mt-4 rounded-lg border border-border bg-surface p-4">
    <div class="mb-2 flex items-center justify-between text-sm">
      ...
      <span class="font-mono text-text">
        ...
        {#if enrichFailed > 0}<span class="text-warning"> ({enrichFailed} failed)</span>{/if}
      </span>
    </div>
...
{#if !enriching && showSuccess && parsedCount > 0 && enrichFailed === 0}
  <div class="mt-4 flex items-start justify-between gap-4 rounded-lg border border-success-border bg-success-bg p-4">
    <div class="flex gap-2.5">
      <span class="text-success">✓</span>
      <div>
        <p class="text-sm font-semibold text-success">Parsed {parsedCount} transactions from {sourceName}</p>
        <p class="mt-0.5 text-sm text-success">{newCount} new · {dupCount} duplicate{dupCount === 1 ? '' : 's'} skipped</p>
      </div>
    </div>
    <button
...
{#if !enriching && parsedCount > 0 && enrichFailed > 0}
  <div class="mt-4 rounded-xl border border-warning-border bg-warning-bg p-4">
    <div class="flex gap-3">
      <span class="mt-0.5 shrink-0 text-warning">⚠</span>
      <div class="flex-1">
        <p class="text-sm font-semibold text-warning">Parsed {parsedCount} transactions from {sourceName}</p>

        <div class="mt-2.5 flex items-center gap-3">
          <div class="h-1.5 w-28 overflow-hidden rounded-full bg-warning-border">
            <div class="h-full bg-positive" style="width: {pricedPct}%"></div>
          </div>
          <p class="text-xs text-warning">
            <strong class="font-semibold text-success">{pricedCount} priced</strong>
            · <strong class="font-semibold text-warning">{enrichFailed} need prices</strong>
          </p>
        </div>

        <p class="mt-2.5 text-sm leading-relaxed text-warning">
        ...
        <button
          class="mt-3 rounded-lg bg-accent px-4 py-2 text-sm font-semibold text-on-accent transition-colors hover:bg-accent/90"
